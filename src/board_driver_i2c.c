...
{
  //Setting clock
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID( I2C_GCLK_ID ) | // Generic Clock 0 (SERCOMx)
  ...
  PM->APBCMASK.reg |= I2C_PM_APBCMASK;
}
...
void i2c_init(uint32_t baud) {
  ...
  pin_set_peripheral_function(I2C_SDA);
  pin_set_peripheral_function(I2C_SCL);
}
...
uint8_t i2c_read(uint8_t address, uint8_t bytes, uint8_t* buf, bool stopBit)
{
  if(bytes == 0) {
    return 0;
  }
